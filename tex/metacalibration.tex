...
\rmcomment{Figure is impossible to read in current form.  I recommend a lot of changes: within a
  given set of figures, use the same vertical axis for both panels in each row, and join the
  panels (no space between them).  Increase font size by a lot; it should be similar to the font
  size in the text of the paper according to the MNRAS editors.  The axis labels need to use the
  same notation as the paper (e.g., $g_2$ instead of ``g2'').  And actually, I think the best thing
  to do would be to completely rearrange this: have a $6\times 2$ figure showing what are currently
  the top 2 figures in each of the 6 panels (so we always show figures related to calibration bias
  together), have another $6\times 2$ figure with the middle two figures in each of the 6 panels (all
the multinomial log likelihood vs. bias plots), and another with shear bias vs PSF ellipticity.
Basically, this will turn a single plot with 36 panels into 3 figures each with 12 panels, where the
12 panels in a given figure are all making the same point.  I realize
this is a lot of work, but the figure is extremely hard to read as-is, and if you make it readable
then it'll be too large for one figure.  Given that this is the key result, we want people to be
able ot understand it and refer to it easily! Also, on the vertical axes, let's not
say ``shear bias'' (which is ambiguous and sounds like it could be multiplicative).  Let's say
something like ``$g_{1,\text{est}}-g_{1,\text{true}}$'' or ``estimated - true shear''. }
}
