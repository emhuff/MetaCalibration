...
\section{Results}
\rmcomment{This section never refers to Table~\ref{table:results}.
  Need to prominently highlight its existence from the start.  Also,
  the figures are all out of order (we've gone from fig 2 to fig 6 for
  example).  And I feel the section is kind of odd in that the
  previous section takes time to step through all branches and what we
  hope to learn from them, yet this section doesn't explicitly refer
  back and explain the lessons learned from each one.}

This section draws inferences about the performance of the
metacalibration algorithm by comparing the results across branches. We
...
realistic morphology branches.  This should not surprise: we have
chosen to perturb and detrend the linear effects of PSF ellipticity, but
other PSF properties may be more significant. The coupling between PSF
morphology and inferred shear will in general depend on the details of
both, as well as the measurement algorithm. We defer further
...
Figure~\ref{fig:noise_bias}. For the typical signal-to-noise ratios
seen in the GREAT3 simulations, biases resulting from correlated noise
appear to scale roughly as the variance of the inital noise field. We
...
section~\ref{sec:model_checking} tends to make a significant impact in
the calibration bias results. The rejected fields (red squares in
...
CGC-noaber and RGC-noaber branches.
...
\subsection{Shape Measurement Algorithms}
We investigate the effects of measurement algorithm by comparing the
...
performance for each method. There is weak evidence for residual KSB
...
algorithms, respectively (where the errors are boostrap estimates).

Another potential driver of residual calibration performance is
...
These trends suggest that the small residual calibration bias in KSB
is driven the nonlinear response of that shape measurement algorithm,
while the residual linear moments calibration is primarily driven by
