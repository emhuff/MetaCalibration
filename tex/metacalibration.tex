...
\item instrumental defects and nonlinearities \footnote{(\nohl{for an overview of new
  detrending and calibration methods introduced to deal with the increased precision requirements
  arising in contemporary weak lensing surveys, see} \citealt{2017PASP..129k4502B}\nohl{)}}
\item star-galaxy separation
\item non-white pixel noise \nohl{(i.e., a pixel noise power spectrum that
  is not flat)}
...
\end{align}
\nohl{This is because the largest displacement of a Fourier mode under a
...
\nohl{By measuring the change in each measured object's ellipticity to an
...
appear in Eq.}~\ref{eqn:calibPars} \nohl{are ensemble responses, however, so
...
the sky, \nohl{and that the linear response of the detector is accurately
...
\nohl{Note that per-object responsivities can be very large in
...
magnitude (as seen in figure}~\ref{fig:calibhist}), \nohl{due to the finite
...
\nohl{For sufficiently large $N_{\rm tot}$, we can treat the histogram likelihood as
...
\end{widetext}
\nohl{This likelihood function is maximized for the estimator}
...
\nohl{If we have the wrong model for the unlensed ellipticity distribution
...
Eq.}~\ref{eqn:mle} \nohl{will be unbiased. In that case we should expect the
...
aberrations in the optical PSF set to zero, \nohl{to ensure that the shapes
...
construction of the counterfactual image. \nohl{The presence of cosmic
...
\nohl{We note that} \citealt{metacalII} \nohl{ deals successfully with
...
shear from a constant-shear field. \nohl{This procedure is applicable to
