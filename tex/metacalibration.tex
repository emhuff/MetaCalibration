...
\includegraphics[width=0.3\linewidth]{./Plots/rgc-noaber-regauss-opt-shear_plots.png}
\includegraphics[width=0.3\linewidth]{./Plots/rgc-noaber-regauss-opt-shear_plots.png}
\caption{Results of metacalibration for regauss on RGC with aberrations. From {\bf right} to {\bf left}: regauss, ksb, moments. Note that we do not have these results for any method yet.}
\end{figure*}
...
\towriteemh{Comment on effects that this method may deal with, which we haven’t tested (selection biases, other sys- tematics detrending)}
