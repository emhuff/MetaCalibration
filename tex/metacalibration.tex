...
\rmcomment{Mention something like ``The existence of these power-law wings may be specific to the
  adopted shape measurement method.  It is possible that other shape measurement methods would
  result in per-object responsivity distributions with properties that are more amenable to simple
  averaging, simplifying the estimate of the ensemble average responsivity.''?}
...
\caption{Effects of the metacalibration algorithm applied to PSF
  ...
  requirements in a realistic experiment. \rmcomment{need to explicitly identify these with the red points/boxes.}}
...
\rmcomment{Figures are still out of order.}

This section draws inferences about the performance of the
metacalibration algorithm by comparing the results across simulation branches. We
discuss the impact of realistic galaxy morphologies, correlated noise,
a heterogeneous PSF, and shape measurement algorithms.
...
Table~\ref{table:results}. Visualizations of the impact of
metacalibration and PSF ellipticity detrending are provided in
Figures~\ref{fig:m_results} and \ref{fig:a_results}.

\rmcomment{You seem to be missing a baseline reporting of the most important results.  You haven't
  yet said ``here's how metacal does in the simplest case'', but rather jump straight to ``galaxy
  morphology doesn't significantly impact how well metacal works'', which seems like putting the
  cart before the horse.  How about a brief subsection called something like ``Basic results''
  stepping through figure 3 and related numbers in table 1?}

\subsection{Galaxy Morphology}
We can compare the performance of the metacalibration algorithm on
model and realistic galaxy morphologies in two cases (RGC-regauss
vs.\ CGC-regauss, and RGC-noaber-regauss vs.\ CGC-noaber-regauss). In
...
chosen to perturb and detrend the linear effects of PSF ellipticity, but
other PSF properties may be more significant and hence our model is not a complete description of
additive systematics. The coupling between PSF
morphology and inferred shear will in general depend on the details of
both, as well as the galaxy morphology and measurement algorithm. We defer further
development of methods to select appropriate control variables for PSF
detrending to future work.
...
be transformed in the counterfactual images into a correlated noise field
with preferred direction and scale. The calibration biases resulting
from the successive increases in initial noise are shown in
Figure~\ref{fig:noise_bias}. \rmcomment{I think this is the first reference to ``increases in
  initial noise'' and I think requires slightly more explanation, i.e., you completely redid the
  entire metacal and measurement process after making versions of the sims with added noise.} For the typical signal-to-noise ratios
seen in the GREAT3 simulations, biases resulting from correlated noise
...
signal-to-noise ratio has been reduced by $50\%$ from the fiducial
GREAT3 values.

The case of the $50\%$ increase is denoted ``CGC-regauss-noisy'' in
...
section~\ref{sec:model_checking} tends significantly improve
the calibration bias after metacalibration. The rejected fields (red squares in
Figure~\ref{fig:psf_trends}) tend to have substantially larger
residual shear calibration biases then the mean field in each branch,
and without the outlier rejection, we see few-percent level
calibration biases in each of these branches. With the likelihood
...
CGC-noaber and RGC-noaber branches. \rmcomment{It's unclear how to make this feasible in reality, if
  the method used in the sims is to reject the worst N\%.  Need some objective cutoff, no?  Is this
...
regauss fields we report on, and so a single $\sim1\sigma$ detection
...
significant trend between PSF and inferred shear.
...
\subsection{Shape Measurement Algorithms}
We investigate the effects of the shape measurement algorithm by comparing the
performance of the regauss, KSB, and linear moments measurement
algorithms on the CGC simulation branch. We see large improvements in
performance for each method after using metacalibration. There is weak evidence for residual KSB
calibration biases, and strong evidence for residual biases in the
linear moments. In the latter case, however, the improvement relative
to the un-metacalibrated case is large: the multiplicative and
additive calibration biases are reduced by factors of $80$ and $51$,
respectively, resulting in performance for metacalibrated linear
moments that is quantitatively superior to that of the
un-metacalibrated KSB.

One potential reason for the difference in performance is the
linearity of the ensemble estimates in the resulting shear. We find
values for $\eta_{nl}$ of $-0.0034\pm0.0004$, $-0.024\pm0.002$, and
$-0.0037\pm0.0004$, for the regauss, KSB, and linear moments
algorithms, respectively (where the errors are boostrap estimates). \rmcomment{It's hard to derive
  meaning from these numbers without some guidance for the reader of what this may mean in practice.}
...
The $\eta_{nl}$ values and the residual additive and multiplicative biases for these methods 
suggest that the small residual calibration bias in KSB
is driven by the nonlinear response of that shape measurement algorithm,
while the residual linear moments calibration bias is primarily driven by
...
\caption{Shear and PSF calibration bias parameters from each of the
  ...
  above their un-calibrated counterparts. \rmcomment{Do you ever compare $m_1$ vs $m_2$ and so on?
    If not, perhaps just show component-averaged $\langle m\rangle$, $\langle a\rangle$, etc.\ to
    make this less busy.}}
\label{table:results}
\end{table*}
...
\end{center}
\caption{Calibration bias results. \rmcomment{Here, as in several places, additive biases are being
    called ``calibration bias''.  I would like to avoid that; ``calibration bias'' commonly means a
    multiplicative factor, so it's confusing to conflate that with additive biases.} Each row shows linear coupling
