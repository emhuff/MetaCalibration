...
we've tested in. In the one branch (CGC-Moments) where it does not
...
less noisy than the un-metacalibrated shapes.
...
see a significant reduction in the impact of the PSF on the inferred
shears.

The remainder of section draws inferences about the performance of the
...
The additive biases resulting from the varying the noise level are
...
section~\ref{sec:model_checking} tends significantly improve the
...
larger residual shear calibration biases then the mean field in each
...
generated by sampling from equation\ref{eqn:multnomial}, for instance
-- but we defer calibration of this technique to future work.
...
quantitatively superior to that of the un-metacalibrated KSB.
...
algorithms, respectively (where the errors are boostrap
estimates). The quadratic correction to the shear response is much
larger for KSB than for the other methods, suggesting that any
problems with the KSB shapes may arise from its more nonlinear shear
response (see, \citealt{2011MNRAS.410.2156V} for a detailed
...
astronomical images in the shear signal, most astronomical imaging, in
