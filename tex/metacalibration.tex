...
estimate of the per-object shear response
\begin{align}
...
\end{align}
\rmcomment{Need to clarify that here $\mathbf{e}$ is not the original one, but rather the one with
  $\Gamma$ for the PSF?}
...
\rmcomment{This sentence just kind of trails off}
...
weight of scale length $r_g$ is used, where $r_g$ is galaxy size. \rmcomment{``Galaxy size'' defined
  how?}
...
estimate from the ensemble-averaged $\hat{M}_i$ values, 
provided that the model for the predominant sources of
...
inference.  \rmcomment{Do we ever actually do this?  There are some subsections in section 4 that
  mention where we do {\em not} need to make a likelihood cut, but I do not see any that explicitly
  say we {\em do} make a likelihood cut.  If we do that, we should say so (and explain the cut).}

We can also devise a simple test for the linearity (in the shear) of
the ensemble estimator. The estimator we use to infer the shear response 
relies on two-sided finite-difference numerical derivatives, but it is
also possible to measure the nonlinear ensemble response by testing for consistency between the 
forward- and backwards finite-difference
...
publicly in association with this paper in two important ways.  First, the
model for systematics was simpler than the one presented below; 
it neglected additive systematics entirely, 
focusing exclusively on multiplicative systematics. Second, the method for 
