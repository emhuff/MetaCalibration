...
def main(argv):
    ...
    #ax1.set_ylim(-1000,1000) # useful range for a1,a2
    ax1.set_xlim(-200,200) # useful range for a1,a2    
    ax1.set_xlabel(r'psf ellipticity biases ($a_1$, $a_2$) $\times 10^{3}$')
    ...
    for i,this_id in enumerate(unique_id):
        ...
        if results[this_raw][0]['a1'] > 200:
            ...
            upper1 = results[this_raw][0]['a1']
            
        if results[this_raw][0]['a2'] > 200:
            upper2 = 200
        else:
            upper2 = results[this_raw][0]['a2']
        
        a1sep = np.abs(ax1.transData.transform((upper1,1))[0] - ax1.transData.transform((results[this_mc][0]['a1'],1))[0])
        a2sep = np.abs(ax1.transData.transform((upper2,1))[0] - ax1.transData.transform((results[this_mc][0]['a2'],1))[0])
    ...
    ax1.axvspan(-2,2,alpha=0.25,facecolor='red')
    fig1.tight_layout()
    fig1.savefig("a_results_linear.pdf",format='pdf')           
    pass
